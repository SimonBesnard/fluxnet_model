...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for step, (features_d, features_s, target, _) in enumerate(self.train_loader):
            ...
            loss = self.loss_fn(pred[:, self.train_loader.dataset.num_warmup_steps:],
    ...
    def predict(self, target_dir, use_training_set=False):
        ...
        ds_pred =[]
        for step, (features_d, features_s, target, site) in enumerate(data_loader):
            ...
            pred_unstandardize = self.unstandardize_target(pred)
            target_unstandardize = self.unstandardize_target(target)
            
            site = np.array(site)
            pred_unstandardize = pred_unstandardize.cpu().numpy()
            target_unstandardize = target_unstandardize.cpu().numpy()            
            ds_pred.append(xr.Dataset({'obs': xr.DataArray(target_unstandardize[:, :, -1], dims = ['site', 'time'], coords=[site, xr_var.time]),
                                       'mod': xr.DataArray(pred_unstandardize[:, :, -1], dims = ['site', 'time'], coords=[site, xr_var.time])}))
            loss = self.loss_fn(
                pred,
                target)
        ...
        pred.attrs = {
            ...
            'description': 'CO2 fluxes prediction',
            'var': xr_var.name,
        ...
        for site_ in pred.site:
            pred.sel(site=site_).to_netcdf(os.path.join(
                target_dir, f'pred_{str(site_.values)}.nc'), mode = 'w')
        print('Done.')
